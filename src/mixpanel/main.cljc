(ns mixpanel.main
  "Kotodama WASM entrypoint for the Mixpanel clean-room actor (L4) — Clojure port.
  ...
  facts (`mixpanel.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mixpanel")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mixpanel.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mixpanel-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mixpanel-compat/ready)
